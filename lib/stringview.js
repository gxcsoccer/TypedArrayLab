...
function StringView(vInput
    ...
    case "object":
        ...
        case Uint8Array:
            ...
            fTAView === Uint32Array ? vInput.buffer.byteLength >>> 2 : fTAView === Uint16Array ? vInput.buffer.byteLength >>> 1 : vInput.buffer.byteLength) ? vInput : new fTAView(vInput.buffer);
            aRaw = nStartIdx === 0 && (!isFinite(nLength) || nLength === nInptLen) ? vInput : vInput.subarray(nStartIdx, isFinite(nLength) ? nLength : nInptLen - nStartIdx);
        ...
        default:
            ...
            aRaw = nStartIdx === 0 && (!isFinite(nLength) || nLength === nInptLen) ? aWhole : aWhole.subarray(nStartIdx, isFinite(nLength) ? nLength : nInptLen - nStartIdx);
        }
        break typeSwitch;
    default:
    ...
    if (nTranscrType < 8) {
        ...
        if (nTranscrType & 4) { /* input is string */
            ...
            nTranscrType ^= this.encoding === "UTF-32" ? 0 : 2; /* ...or...: nTranscrType ^= Number(this.encoding !== "UTF-32") << 1; */
...
StringView.loadUTF8CharCode = function(aChars, nIdx) {
    ...
    return nPart > 251 && nPart < 254 && nIdx + 5 < nLen ? /* (nPart - 252 << 32) is not possible in ECMAScript! So...: */
    /* six bytes */
    (nPart - 252) * 1073741824 + (aChars[nIdx + 1] - 128 << 24) + (aChars[nIdx + 2] - 128 << 18) + (aChars[nIdx + 3] - 128 << 12) + (aChars[nIdx + 4] - 128 << 6) + aChars[nIdx + 5] - 128 : nPart > 247 && nPart < 252 && nIdx + 4 < nLen ? /* five bytes */
    (nPart - 248 << 24) + (aChars[nIdx + 1] - 128 << 18) + (aChars[nIdx + 2] - 128 << 12) + (aChars[nIdx + 3] - 128 << 6) + aChars[nIdx + 4] - 128 : nPart > 239 && nPart < 248 && nIdx + 3 < nLen ? /* four bytes */
    (nPart - 240 << 18) + (aChars[nIdx + 1] - 128 << 12) + (aChars[nIdx + 2] - 128 << 6) + aChars[nIdx + 3] - 128 : nPart > 223 && nPart < 240 && nIdx + 2 < nLen ? /* three bytes */
    (nPart - 224 << 12) + (aChars[nIdx + 1] - 128 << 6) + aChars[nIdx + 2] - 128 : nPart > 191 && nPart < 224 && nIdx + 1 < nLen ? /* two bytes */
    (nPart - 192 << 6) + aChars[nIdx + 1] - 128 : /* one byte */
    nPart;
...
StringView.putUTF8CharCode = function(aTarget, nChar, nPutAt) {
    ...
    if (nChar < 0x80 /* 128 */ ) { /* one byte */
        aTarget[nIdx++] = nChar;
    } else if (nChar < 0x800 /* 2048 */ ) { /* two bytes */
        aTarget[nIdx++] = 0xc0 /* 192 */
        + (nChar >>> 6);
        aTarget[nIdx++] = 0x80 /* 128 */
        + (nChar & 0x3f /* 63 */ );
    } else if (nChar < 0x10000 /* 65536 */ ) { /* three bytes */
        aTarget[nIdx++] = 0xe0 /* 224 */
        + (nChar >>> 12);
        aTarget[nIdx++] = 0x80 /* 128 */
        + ((nChar >>> 6) & 0x3f /* 63 */ );
        aTarget[nIdx++] = 0x80 /* 128 */
        + (nChar & 0x3f /* 63 */ );
    } else if (nChar < 0x200000 /* 2097152 */ ) { /* four bytes */
        aTarget[nIdx++] = 0xf0 /* 240 */
        + (nChar >>> 18);
        aTarget[nIdx++] = 0x80 /* 128 */
        + ((nChar >>> 12) & 0x3f /* 63 */ );
        aTarget[nIdx++] = 0x80 /* 128 */
        ...
        aTarget[nIdx++] = 0x80 /* 128 */
        + (nChar & 0x3f /* 63 */ );
    } else if (nChar < 0x4000000 /* 67108864 */ ) { /* five bytes */
        aTarget[nIdx++] = 0xf8 /* 248 */
        + (nChar >>> 24);
        aTarget[nIdx++] = 0x80 /* 128 */
        + ((nChar >>> 18) & 0x3f /* 63 */ );
        aTarget[nIdx++] = 0x80 /* 128 */
    ...
    { /* 2147483647 */
        ...
        + /* (nChar >>> 32) is not possible in ECMAScript! So...: */
        (nChar / 1073741824);
        aTarget[nIdx++] = 0x80 /* 128 */
        + ((nChar >>> 24) & 0x3f /* 63 */ );
        aTarget[nIdx++] = 0x80 /* 128 */
...
StringView.loadUTF16CharCode = function(aChars, nIdx) { /* UTF-16 to DOMString decoding algorithm */
    var nFrstChr = aChars[nIdx];
    return nFrstChr > 0xD7BF /* 55231 */
    && nIdx + 1 < aChars.length ? (nFrstChr - 0xD800 /* 55296 */ << 10) + aChars[nIdx + 1] + 0x2400 /* 9216 */
    : nFrstChr;
};

StringView.putUTF16CharCode = function(aTarget, nChar, nPutAt) {
    var nIdx = nPutAt;
    if (nChar < 0x10000 /* 65536 */ ) { /* one double-byte */
        aTarget[nIdx++] = nChar;
    } else { /* two double-bytes */
        aTarget[nIdx++] = 0xD7C0 /* 55232 */
        + (nChar >> 10);
        aTarget[nIdx++] = 0xDC00 /* 56320 */
        + (nChar & 0x3FF /* 1023 */ );
    }
    return nIdx;
};
...
StringView.prototype.makeIndex = function(nEndOffsetChr, nSkipOffsetIdx) {
    ...
    taTarget = this.rawData,
        nChrEnd, nIdxEnd = 0,
        nRawLength = taTarget.length,
        nStartIdx = nSkipOffsetIdx || 0,
        nStopAtChr = isNaN(nEndOffsetChr) ? Infinity : nEndOffsetChr;

    if (nEndOffsetChr + 1 > taTarget.length) {
    ...
    case "UTF-16":
        ...
        for (nChrEnd = nStartIdx; nIdxEnd < nRawLength && nChrEnd < nStopAtChr; nChrEnd++) {
            nIdxEnd += taTarget[nIdxEnd] > 0xD7BF /* 55231 */
            && nIdxEnd + 1 < taTarget.length ? 2 : 1;
        }
...
StringView.prototype.toBase64 = function(bWholeBuffer) {
    ...
    return bWholeBuffer ? StringView.bytesToBase64(
    this.rawData.constructor === Uint8Array ? this.bufferView : new Uint8Array(this.buffer)) : StringView.bytesToBase64(
    this.rawData.constructor === Uint8Array ? this.rawData : new Uint8Array(
    this.buffer, this.rawData.byteOffset, this.rawData.constructor === Uint16Array ? this.rawData.length << 1 : this.rawData.length << 2));
...
StringView.prototype.subview = function(nCharOffset /* optional */ , nCharLength /* optional */ ) {
    ...
    nRawLen, nCharStart, nCharStop, bVariableLen = this.encoding === "UTF-8" || this.encoding === "UTF-16",
        nByteOffset = nCharOffset,
        taSuperset = this.rawData,
        nStringLength, nTableLen = taSuperset.length;
    ...
    if (bVariableLen) {
        ...
        nRawLen = this.makeIndex(nCharStop, nByteOffset) - nByteOffset;
    } else {
...
StringView.prototype.valueOf = StringView.prototype.toString = function() {
    ...
    if (this.encoding !== "UTF-8" && this.encoding !== "UTF-16") { /* ASCII, UTF-32 or BinaryString to DOMString */
